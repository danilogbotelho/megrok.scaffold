...
class ControllerGrokker(martian.ClassGrokker):
        ...
        for action in ['add', 'edit', 'display', 'list']:
            if scaffold.get(action, False):
                ...
                ret = executer(factory, config, context,
                    ...
                    scaffold[action], scaffold['aspage'],
                    permissions.get(action, None))
    ...
    def exec_add(self, factory, config, context, layer, name, iface,
                 aspage, permission):
        if not name:
    ...
    def exec_list(self, factory, config, context, layer, name, iface,
                  aspage, permission):
        if not name:
    ...
    def exec_edit(self, factory, config, context, layer, name, iface,
                  aspage, permission):
        if not name:
            name = 'edit'
    ...
    def exec_display(self, factory, config, context, layer, name, iface,
                     aspage, permission):
        if not name:
            name = 'index'
